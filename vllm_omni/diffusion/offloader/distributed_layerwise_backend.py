        ...
        for dtype, name2weights in dtype_grouped.items():
            ...
            for name, t in name2weights.items():
                ...
                storage_numel = (
                    ...
                    + sum(
                        (size - 1) * axis_stride
                        for size, axis_stride in zip(local_t.shape, stride)
                    )
                )
                weights_with_local.append(
                    (name, t, local_t, storage_numel, stride)
                )

            total_numel = sum(
                storage_numel
                for _, _, _, storage_numel, _ in weights_with_local
            )
