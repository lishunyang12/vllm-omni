...
def jobs(windows=21, tiles=28, world=8):
    if min(windows, tiles, world) < 1:
        raise ValueError("VAE schedule dimensions must be positive")
    rounds = []
    ...
    assert sum(counts) == windows * tiles
...
def decode_pairs(model, latent, group, callback, *, temporal_cat_dtype=None, gather_stream=None):
    ...
    assert (callback is not None) == (group.rank_in_group == 0)
    spatial, y_overlap, x_overlap = geometry(model, latent)
    schedule = jobs(world=group.world_size)
        ...
        for round_id, round_jobs in enumerate(schedule):
            ...
            for slot, (window, tile) in enumerate(round_jobs[rank]):
                ...
                try:
                    ...
                    with torch.profiler.record_function(f"h3.vae.pair.tile.{window}.{tile}"):
            ...
            with torch.profiler.record_function(f"h3.vae.pair.gather.{round_id}"):
    ...
    return dict(rank=rank, tile_calls=tile_count, gathers=len(schedule), windows=21, frames=362)
...
@contextmanager
def mixed_decode(model, latent, rank, world=8):
    ...
    try:
        ...
        assert stats["decoder_calls"] == len(plan)
