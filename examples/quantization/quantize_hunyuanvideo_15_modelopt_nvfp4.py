...
"""Quantize HunyuanVideo-1.5 (480p T2V) to a ModelOpt NVFP4 Hugging Face checkpoint.

Near-identical to quantize_hunyuanvideo_15_modelopt_fp8.py but uses ModelOpt's
NVFP4_DEFAULT_CFG (W4A4 FP4 with 16-element group-wise scales along the input
dim) instead of FP8_DEFAULT_CFG. Produces ~4× weight compression vs BF16.
...
Example:
    python examples/quantization/quantize_hunyuanvideo_15_modelopt_nvfp4.py \\
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=int,
        default=15,
        help="Denoising steps per calibration prompt. More steps = better activation coverage. "
        "Default 15 is a good quality/time tradeoff for NVFP4.",
    )
    p.add_argument(
        "--calib-size",
        type=int,
        default=16,
        help="How many prompts to calibrate with. 16+ recommended for NVFP4 to cover outlier "
        "activations (FP4 is precision-limited, so scale estimates matter more than FP8).",
    )
...
# Same precision-sensitive layers as the FP8 variant.
...
def _disable_known_problematic_quantizers(mtq: Any, backbone: torch.nn.Module) -> None:
...
def main() -> None:
    ...
    _disable_known_problematic_quantizers(mtq, backbone)
