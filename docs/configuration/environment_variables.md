...
| `VLLM_OMNI_VIDEO_SYNC_TIMEOUT` | Float seconds; unset by default (wait for completion) | Synchronous Videos API; read when the API server module imports | Optional operator deadline. Expiration returns HTTP 504. A non-float raises `ValueError` during import. | Experimental |
